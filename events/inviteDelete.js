module.exports = async (client, invite) => {
  ...
  try {
    if (!db.get(`guild_${guild.id}_logging_inv-delete_enabled`) && !db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
      return;
    } else {
      if (db.get(`guild_${guild.id}_logging_all_enabled`) === true && db.get(`guild_${guild.id}_logging_inv-delete_enabled`) === true) {
				var setChannel = db.get(`guild_${guild.id}_logging_inv-delete_channel`);
			} else if (db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
				setChannel = db.get(`guild_${guild.id}_logging_all_channel`);
			} else if (db.get(`guild_${guild.id}_logging_inv-delete_enabled`) === true) {
				setChannel = db.get(`guild_${guild.id}_logging_inv-delete_channel`);
			}
      if (msgtyp === 'normal') {
        if (detail === '1') {
          client.channels.cache.get(setChannel).send(`**Invite Deleted**\nCode: \`${invite.code}\`Temporary: ${invite.temporary}`);
        } else if (detail === '2') {
