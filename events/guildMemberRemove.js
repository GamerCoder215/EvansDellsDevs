module.exports = async (client, member) => {
  ...
  try {
    if (!db.get(`guild_${guild.id}_logging_sr-memrem_enabled`) && !db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
      return;
    } else {
      if (db.get(`guild_${guild.id}_logging_all_enabled`) === true && db.get(`guild_${guild.id}_logging_sr-memrem_enabled`) === true) {
				var setChannel = db.get(`guild_${guild.id}_logging_sr-memrem_channel`);
			} else if (db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
				setChannel = db.get(`guild_${guild.id}_logging_all_channel`);
			} else if (db.get(`guild_${guild.id}_logging_sr-memrem_enabled`) === true) {
				setChannel = db.get(`guild_${guild.id}_logging_sr-memrem_channel`);
			}
      if (msgtyp === 'normal') {
        if (detail === '1') {
          client.channels.cache.get(setChannel).send(`**User Left**\nUser: ${member.user.tag}\n\nMembers Left: ${guild.memberCount}`);
        } else if (detail === '2') {
        ...
        if (detail === '1') {
          ...
          .setDescription(`Members Left: ${guild.memberCount}`)
          .addFields(
