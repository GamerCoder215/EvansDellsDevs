module.exports = async (client, guild, user) => {
  ...
  try {
		if (!db.get(`guild_${guild.id}_logging_sr-bremove_enabled`) && !db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
			return;
		} else {
      if (db.get(`guild_${guild.id}_logging_all_enabled`) === true && db.get(`guild_${guild.id}_logging_sr-bremove_enabled`) === true) {
      var setChannel = db.get(`guild_${guild.id}_logging_sr-bremove_channel`);
    } else if (db.get(`guild_${guild.id}_logging_all_enabled`) === true) {
      setChannel = db.get(`guild_${guild.id}_logging_all_channel`);
    } else if (db.get(`guild_${guild.id}_logging_sr-bremove_enabled`) === true) {
      setChannel = db.get(`guild_${guild.id}_logging_sr-bremove_channel`);
      }
      if (msgtyp === 'normal') {
        if (detail === '1') {
          client.channels.cache.get(setChannel).send(`**User Unbanned**\nUser: ${user.tag}\nUser ID: ${user.id}\n\nThere are now **${guild.memberCount}** members!`);
        } else if (detail === '2') {
        ...
        if (detail === '1') {
          ...
          .setDescription(`There are now **${guild.memberCount}** members!`)
          .addFields(
