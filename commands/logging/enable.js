module.exports = {
  ...
  var sucessMessages = [
    ...
    .setDescription('This event is already enabled!')
    .setColor(config.red)
    ...
    try {
      ...
      if (args[0] === 'all') {
        if (db.get(`guild_${message.guild.id}_logging_all_enabled`) === true) return message.channel.send(alreadyEnabled);
        db.set(`guild_${message.guild.id}_logging_all_enabled`, true);
        message.channel.send(actionSucessful);
      } else {
        if (db.get(`guild_${message.guild.id}_logging_${args[0]}_enabled`) === true) return message.channel.send(alreadyEnabled);
        db.set(`guild_${message.guild.id}_logging_${args[0]}_enabled`, true);
        message.channel.send(actionSucessful);
      }
    } catch (error) {
      console.error(error);
