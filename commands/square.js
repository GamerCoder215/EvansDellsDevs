	...
	async run(client, message, args) {
		...
		try {
			if (!args[0]) {
				...
				var squaredNumber = args[0] * args[0]
				const squaredEmbed = new Discord.MessageEmbed()
				.setAuthor(message.author.username, message.author.displayAvatarURL({ dynamic: true, format: 'png', size: 1024}))
				.setDescription(`Your answer is \`${squaredNumber}\``)
				.setColor(config.gold)
				.setFooter(config.name, config.icon)
				.setTimestamp();
				message.channel.send(squaredEmbed);
			}
		} catch (error) {
			console.error(error);
