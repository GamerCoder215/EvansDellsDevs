module.exports = {
	name: 'mutechannel',
	description: 'Mute a specified channel.',
	guildOnly: true,
	aliases: ['channelmute', 'channelm', 'mutech', 'chmute'],
	async run(client, message, args) {
		...
		.setDescription('This channel is already muted. Please use ?unmutechannel <channel> [duration].')
		.setColor(config.red)
		.setFooter(config.name, config.icon)
		.setTimestamp();
		// Invalid Args Embed
		...
		try {
			if (!message.member.hasPermission('MANAGE_MESSAGES')) {
				...
				if (!args[0]) {
					...
					if (db.get(`channel_${getID(args[0])}_muted`) === true) {
						return message.channel.send(alreadyMuted);
					} else {
						if (!args[1]) {
						db.set(`channel_${getID(args[0])}_muted`, true)
						message.channel.send(actionSucessful);
						client.channels.cache.get(mutedChannel.id).send(`This channel was muted by ${message.author.username}.`)
						} else {
						db.set(`channel_${getID(args[0])}_muted`, true)
						message.channel.send(actionSucessful);
						...
						setTimeout(() => {
							...
							client.channels.cache.get().send(`This channel was unmuted after \`${args[1]} \`minutes.`)
						}, 1000 * 60 * args[1]);
						}
					}
				}
