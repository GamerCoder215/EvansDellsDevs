...
for (const file of advtoolsCommandFiles) {
	...
	db.set(`module_${command.name}`, `advtools`);
	client.commands.set(command.name, command)
	console.log(`Advanced Tools Command \x1b[31m${command.name}\x1b[0m loaded.`)
	fileCount++;
	commandCount++;
}
...
process.on('unhandledRejection', error => {
	console.error(`\x1b[31mERROR \x1b[33mUnhandled Rejection Error: \x1b[35m${error}\x1b[0m`)
})
